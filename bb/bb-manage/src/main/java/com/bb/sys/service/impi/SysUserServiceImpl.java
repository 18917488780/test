...
import com.bb.pj.sys.dao.SysUserDao;
import com.bb.pj.sys.dao.SysUserRoleDao;
import com.bb.pj.sys.entity.SysUser;
import com.bb.pj.sys.vo.SysUserDeptVo;
import com.bb.sys.service.SysUserService;
...
public class SysUserServiceImpl implements SysUserService {
	...
	@Transactional
	@Override public int saveObject(SysUser entity, Integer[] roleIds) {
		//1.验证数据合法性 
		if(entity==null) throw new ServiceException("保存对象不能为空");
		if(StringUtils.isEmpty(entity.getUsername())) throw new
		ServiceException("用户名不能为空"); if(StringUtils.isEmpty(entity.getPassword()))
			throw new ServiceException("密码不能为空"); if(roleIds==null || roleIds.length==0)
				throw new ServiceException("至少要为用户分配角色");

			//2.将数据写入数据库
			String salt=UUID.randomUUID().toString(); entity.setSalt(salt);
			//加密(先了解,讲shiro时再说) 
			SimpleHash sHash= new
					SimpleHash("MD5",entity.getPassword(), salt);
			entity.setPassword(sHash.toHex());

			int rows=sysUserDao.insertObject(entity); 
			int urows = sysUserRoleDao.insertObjects(
					entity.getId(), roleIds);//"1,2,3,4"; //3.返回结果
